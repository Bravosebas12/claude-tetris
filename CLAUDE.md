...
- **`game.js`** — all game logic, in one file:
  ...
  - **Pieces**: the 7 tetrominoes as square matrices in `PIECES`; rotation is `rotateCW` (transpose + reverse), collision is `collide`, and `tryRotate` implements basic wall kicks by retrying rotation at `x` offsets `[0, -1, 1, -2, 2]`.
