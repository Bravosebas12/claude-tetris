...
- `index.html` — DOM structure: the main `<canvas id="board">` (300×600, 10×20 grid of 30px blocks), the `<canvas id="next-canvas">` preview, HUD elements (`#score`, `#lines`, `#timer`, `#level`, `#combo`), the mode/theme/sound toggle buttons, and the pause/game-over/challenge-result overlay.
...
- **Locking a piece**: `lockPiece()` → `merge()` (writes the piece into `board`) → `clearLines(neutralTurn)` → (if the challenge was just won, `clearLines()` already set `gameOver` and `lockPiece()` returns without spawning) → `spawn()` (promotes `next` to `current`, generates a new `next`; if the new piece immediately collides, calls `endGame()`). `neutralTurn` is `true` when the locked piece was a `BOMB`, so an empty-landing bomb doesn't break the combo streak.
...
- **Win**: detected in `clearLines()`, right after `lines += cleared`/`updateHUD()` — the only place `lines` grows, so it catches both normal and bomb-triggered clears. Sets a "¡GANASTE!" `banner` and calls `finishChallenge(true)`.
...
- `SFX` is a plain object of fire-and-forget functions (`move`, `rotate`, `softDrop`, `hardDrop`, `lock`, `clear(n)`, `tetris`, `combo(mult)`, `levelUp`, `win`, `timeUp`, `bomb`, `gameOver`) — `SFX.combo(mult)` is the pitch-rises-with-the-streak signature sound; `SFX.win()`/`SFX.timeUp()` are the challenge-mode victory fanfare and time-out sound, both built from the same `tone()` primitive. None of them ever `await` or block the game loop.
