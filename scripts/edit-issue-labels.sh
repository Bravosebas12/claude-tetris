...
#
# The issue number is read from the workflow event payload, so it is always
# bound to the issue that triggered the workflow (can't be pointed elsewhere).
#
...
# Fetch valid labels from the repo and only apply ones that actually exist
